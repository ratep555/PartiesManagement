...
namespace Infrastructure.Services
{
    public class EmailService : IEmailService
    {
        ...
        public void GeneratePdf2(int orderNo, decimal amount, string name)
        {
            var accounts = _context.Accounts.ToList();
        ...
        public async Task SendEmailAsync2(string toEmail, string subject, string content, int orderNo)
        {
            var apiKey = _config["SendGridAPIKey"];
            var client = new SendGridClient(apiKey);
            var from = new EmailAddress("petar.sardelic@outlook.com", "Kidsparties.com");
            var to = new EmailAddress(toEmail);
            var msg = MailHelper.CreateSingleEmail(from, to, subject, content, content);

            using (var fileStream = File.OpenRead("C:\\Users\\petar\\source\\repos\\" + orderNo.ToString() + ".pdf"))
            {
                await msg.AddAttachmentAsync("TestPDF7.pdf", fileStream);
                var response = await client.SendEmailAsync(msg);
            }
        }
    }
